package cn.tootoo.soa.oms.subordercancel;
...
import cn.tootoo.soa.oms.subordercancel.delegate.AbstractSubOrderCancelDelegate;
import cn.tootoo.soa.oms.subordercancel.delegate.SubOrderCancelDelegateFactory;
 ...
 * 接口remark：
 */
public final class SubOrderCancelImpl extends BaseService {
    
    static {
        try {
            Class.forName("cn.tootoo.soa.oms.subordercancel.delegate.SubOrderCancelDelegate");
        } catch (ClassNotFoundException e) {
            Logger.getLogger(SubOrderCancelImpl.class.getName()).error("cn.tootoo.soa.oms.subordercancel.delegate.SubOrderCancelDelegate未找到！", e);
        }
    }
     ...
     * 日志对象。
     */
    private Logger logger = Logger.getLogger(SubOrderCancelImpl.class);
    
    /**
     * 构造函数。
     */
    public SubOrderCancelImpl() {
        /*如果捕获到未知异常，则在此处统一处理。*/
    ...
    public BaseOutputBean doService(BaseInputBean inputBean) throws Exception {
        ...
        /*实例化返回结果数据体。*/
        final OmsSubOrderCancelOutputData outputData = new OmsSubOrderCancelOutputData();
        outputBean.setOutputData(outputData);
        /*实例化服务委托者。*/
        final AbstractSubOrderCancelDelegate delegate = SubOrderCancelDelegateFactory.createDelegate();
        /*将生成服务执行标识设置到服务委托者中。*/
