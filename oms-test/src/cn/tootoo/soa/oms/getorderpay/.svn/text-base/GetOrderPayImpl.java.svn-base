package cn.tootoo.soa.oms.getorderpay;
...
import cn.tootoo.soa.oms.getorderpay.delegate.AbstractGetOrderPayDelegate;
import cn.tootoo.soa.oms.getorderpay.delegate.GetOrderPayDelegateFactory;
 ...
 * 接口remark：
 */
public final class GetOrderPayImpl extends BaseService {
    
    static {
        try {
            Class.forName("cn.tootoo.soa.oms.getorderpay.delegate.GetOrderPayDelegate");
        } catch (ClassNotFoundException e) {
            Logger.getLogger(GetOrderPayImpl.class.getName()).error("cn.tootoo.soa.oms.getorderpay.delegate.GetOrderPayDelegate未找到！", e);
        }
    }
     ...
     * 日志对象。
     */
    private Logger logger = Logger.getLogger(GetOrderPayImpl.class);
    
    /**
     * 构造函数。
     */
    public GetOrderPayImpl() {
        /*如果捕获到未知异常，则在此处统一处理。*/
    ...
    public BaseOutputBean doService(BaseInputBean inputBean) throws Exception {
        ...
        /*实例化返回结果数据体。*/
        final OmsGetOrderPayOutputData outputData = new OmsGetOrderPayOutputData();
        outputBean.setOutputData(outputData);
        /*实例化服务委托者。*/
        final AbstractGetOrderPayDelegate delegate = GetOrderPayDelegateFactory.createDelegate();
        /*将生成服务执行标识设置到服务委托者中。*/
