package cn.tootoo.soa.oms.ordercancel;
...
import cn.tootoo.soa.oms.ordercancel.delegate.AbstractOrderCancelDelegate;
import cn.tootoo.soa.oms.ordercancel.delegate.OrderCancelDelegateFactory;
 ...
 * 接口remark：
 */
public final class OrderCancelImpl extends BaseService {
    
    static {
        try {
            Class.forName("cn.tootoo.soa.oms.ordercancel.delegate.OrderCancelDelegate");
        } catch (ClassNotFoundException e) {
            Logger.getLogger(OrderCancelImpl.class.getName()).error("cn.tootoo.soa.oms.ordercancel.delegate.OrderCancelDelegate未找到！", e);
        }
    }
     ...
     * 日志对象。
     */
    private Logger logger = Logger.getLogger(OrderCancelImpl.class);
    
    /**
     * 构造函数。
     */
    public OrderCancelImpl() {
        /*如果捕获到未知异常，则在此处统一处理。*/
    ...
    public BaseOutputBean doService(BaseInputBean inputBean) throws Exception {
        ...
        /*实例化返回结果数据体。*/
        final OmsOrderCancelOutputData outputData = new OmsOrderCancelOutputData();
        outputBean.setOutputData(outputData);
        /*实例化服务委托者。*/
        final AbstractOrderCancelDelegate delegate = OrderCancelDelegateFactory.createDelegate();
        /*将生成服务执行标识设置到服务委托者中。*/
