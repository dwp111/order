package cn.tootoo.soa.oms.getorderdetail;
...
import cn.tootoo.soa.oms.getorderdetail.delegate.AbstractGetOrderDetailDelegate;
import cn.tootoo.soa.oms.getorderdetail.delegate.GetOrderDetailDelegateFactory;
 ...
 */
public final class GetOrderDetailImpl extends BaseService {
    
    static {
        try {
            Class.forName("cn.tootoo.soa.oms.getorderdetail.delegate.GetOrderDetailDelegate");
        } catch (ClassNotFoundException e) {
            Logger.getLogger(GetOrderDetailImpl.class.getName()).error("cn.tootoo.soa.oms.getorderdetail.delegate.GetOrderDetailDelegate未找到！", e);
        }
    }
     ...
     * 日志对象。
     */
    private Logger logger = Logger.getLogger(GetOrderDetailImpl.class);
    
    /**
     * 构造函数。
     */
    public GetOrderDetailImpl() {
        /*如果捕获到未知异常，则在此处统一处理。*/
    ...
    public BaseOutputBean doService(BaseInputBean inputBean) throws Exception {
        ...
        /*实例化返回结果数据体。*/
        final OmsGetOrderDetailOutputData outputData = new OmsGetOrderDetailOutputData();
        outputBean.setOutputData(outputData);
        /*实例化服务委托者。*/
        final AbstractGetOrderDetailDelegate delegate = GetOrderDetailDelegateFactory.createDelegate();
        /*将生成服务执行标识设置到服务委托者中。*/
