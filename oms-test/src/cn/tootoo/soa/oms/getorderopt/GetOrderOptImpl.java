package cn.tootoo.soa.oms.getorderopt;
...
import cn.tootoo.soa.oms.getorderopt.delegate.AbstractGetOrderOptDelegate;
import cn.tootoo.soa.oms.getorderopt.delegate.GetOrderOptDelegateFactory;
 ...
 */
public final class GetOrderOptImpl extends BaseService {
    
    static {
        try {
            Class.forName("cn.tootoo.soa.oms.getorderopt.delegate.GetOrderOptDelegate");
        } catch (ClassNotFoundException e) {
            Logger.getLogger(GetOrderOptImpl.class.getName()).error("cn.tootoo.soa.oms.getorderopt.delegate.GetOrderOptDelegate未找到！", e);
        }
    }
     ...
     * 日志对象。
     */
    private Logger logger = Logger.getLogger(GetOrderOptImpl.class);
    
    /**
     * 构造函数。
     */
    public GetOrderOptImpl() {
        /*如果捕获到未知异常，则在此处统一处理。*/
    ...
    public BaseOutputBean doService(BaseInputBean inputBean) throws Exception {
        ...
        /*实例化返回结果数据体。*/
        final OmsGetOrderOptOutputData outputData = new OmsGetOrderOptOutputData();
        outputBean.setOutputData(outputData);
        /*实例化服务委托者。*/
        final AbstractGetOrderOptDelegate delegate = GetOrderOptDelegateFactory.createDelegate();
        /*将生成服务执行标识设置到服务委托者中。*/
