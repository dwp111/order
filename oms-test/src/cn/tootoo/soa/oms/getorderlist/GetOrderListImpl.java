package cn.tootoo.soa.oms.getorderlist;
...
import cn.tootoo.soa.oms.getorderlist.delegate.AbstractGetOrderListDelegate;
import cn.tootoo.soa.oms.getorderlist.delegate.GetOrderListDelegateFactory;
 ...
 */
public final class GetOrderListImpl extends BaseService {
    
    static {
        try {
            Class.forName("cn.tootoo.soa.oms.getorderlist.delegate.GetOrderListDelegate");
        } catch (ClassNotFoundException e) {
            Logger.getLogger(GetOrderListImpl.class.getName()).error("cn.tootoo.soa.oms.getorderlist.delegate.GetOrderListDelegate未找到！", e);
        }
    }
     ...
     * 日志对象。
     */
    private Logger logger = Logger.getLogger(GetOrderListImpl.class);
    ...
    public BaseOutputBean doService(BaseInputBean inputBean) throws Exception {
        ...
        /*实例化返回结果数据体。*/
        final OmsGetOrderListOutputData outputData = new OmsGetOrderListOutputData();
        outputBean.setOutputData(outputData);
        /*实例化服务委托者。*/
        final AbstractGetOrderListDelegate delegate = GetOrderListDelegateFactory.createDelegate();
        /*将生成服务执行标识设置到服务委托者中。*/
