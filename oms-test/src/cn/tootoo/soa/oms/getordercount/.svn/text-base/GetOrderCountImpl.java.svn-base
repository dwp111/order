package cn.tootoo.soa.oms.getordercount;
...
import cn.tootoo.soa.oms.getordercount.delegate.AbstractGetOrderCountDelegate;
import cn.tootoo.soa.oms.getordercount.delegate.GetOrderCountDelegateFactory;
 ...
 * 接口remark：
 */
public final class GetOrderCountImpl extends BaseService {
    
    static {
        try {
            Class.forName("cn.tootoo.soa.oms.getordercount.delegate.GetOrderCountDelegate");
        } catch (ClassNotFoundException e) {
            Logger.getLogger(GetOrderCountImpl.class.getName()).error("cn.tootoo.soa.oms.getordercount.delegate.GetOrderCountDelegate未找到！", e);
        }
    }
     ...
     * 日志对象。
     */
    private Logger logger = Logger.getLogger(GetOrderCountImpl.class);
    
    /**
     * 构造函数。
     */
    public GetOrderCountImpl() {
        /*如果捕获到未知异常，则在此处统一处理。*/
    ...
    public BaseOutputBean doService(BaseInputBean inputBean) throws Exception {
        ...
        /*实例化返回结果数据体。*/
        final OmsGetOrderCountOutputData outputData = new OmsGetOrderCountOutputData();
        outputBean.setOutputData(outputData);
        /*实例化服务委托者。*/
        final AbstractGetOrderCountDelegate delegate = GetOrderCountDelegateFactory.createDelegate();
        /*将生成服务执行标识设置到服务委托者中。*/
