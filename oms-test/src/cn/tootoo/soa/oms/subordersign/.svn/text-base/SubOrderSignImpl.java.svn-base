package cn.tootoo.soa.oms.subordersign;
...
import cn.tootoo.soa.oms.subordersign.delegate.AbstractSubOrderSignDelegate;
import cn.tootoo.soa.oms.subordersign.delegate.SubOrderSignDelegateFactory;
 ...
 * 接口remark：
 */
public final class SubOrderSignImpl extends BaseService {
    
    static {
        try {
            Class.forName("cn.tootoo.soa.oms.subordersign.delegate.SubOrderSignDelegate");
        } catch (ClassNotFoundException e) {
            Logger.getLogger(SubOrderSignImpl.class.getName()).error("cn.tootoo.soa.oms.subordersign.delegate.SubOrderSignDelegate未找到！", e);
        }
    }
     ...
     * 日志对象。
     */
    private Logger logger = Logger.getLogger(SubOrderSignImpl.class);
    
    /**
     * 构造函数。
     */
    public SubOrderSignImpl() {
        /*如果捕获到未知异常，则在此处统一处理。*/
    ...
    public BaseOutputBean doService(BaseInputBean inputBean) throws Exception {
        ...
        /*实例化返回结果数据体。*/
        final OmsSubOrderSignOutputData outputData = new OmsSubOrderSignOutputData();
        outputBean.setOutputData(outputData);
        /*实例化服务委托者。*/
        final AbstractSubOrderSignDelegate delegate = SubOrderSignDelegateFactory.createDelegate();
        /*将生成服务执行标识设置到服务委托者中。*/
